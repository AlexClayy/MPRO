...
function Blackjack() {
    ...
    async function calculateWinner(value) {
        ...
        //     setUser(updatedUser);
        // } else if (playerValue >= 22 && playerValue < value || value <= 21) {
        //     setWinner(': Dealer Wins');
        // } else if (value >= 21 && playerValue <= 21){
        ...
        if (playerValue > 21) {
            ...
            const updatedUser = await updateBalance(betAmount);
            setUser(updatedUser);
        } else if (playerValue > 21 && value > 21){
            setWinner(': Draw');
            const updatedUser = await updateBalance(betAmount);
            setUser(updatedUser);
            ...
            const updatedUser = await updateBalance(2 * betAmount);
            setUser(updatedUser);
        } else if(playerValue <= 21 && value < playerValue){
            setWinner(': Player Wins!');
            const updatedUser = await updateBalance(2 * betAmount);
            setUser(updatedUser);
        } else {
